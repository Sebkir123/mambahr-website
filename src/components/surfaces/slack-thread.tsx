...
type SlackThreadProps = {
  ...
  messages: Message[]
  compact?: boolean
  /** Skip the typing animation and render the full thread immediately. */
  instant?: boolean
}

export default function SlackThread({ channel, messages, compact, instant }: SlackThreadProps) {
  // Animate Mamba replies: hold them back, show typing indicator, then reveal.
  // When instant=true, render the final state immediately (good for hero mockups
  // where the typing animation lingers and makes the agent look stuck).
  const ref = useRef<HTMLDivElement>(null)
  const [phase, setPhase] = useState<'idle' | 'typing' | 'sent'>(instant ? 'sent' : 'idle')
    ...
    const obs = new IntersectionObserver(
      ([e]) => {
        if (e.isIntersecting && phase === 'idle') {
          ...
          const t2 = setTimeout(() => setPhase('sent'), 400 + 900)
              ...
              <div key={`typing-${i}`} className="live-arrive" style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0, paddingTop: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: compact ? 12 : 13, fontWeight: 700, color: 'var(--text)' }}>{msg.name}</span>
                    <span style={{ fontSize: 10, fontWeight: 600, background: 'var(--gold-tint)', color: 'var(--gold-dark)', border: '1px solid var(--gold-light)', borderRadius: 3, padding: '1px 5px' }}>APP</span>
            ...
            >
              ...
              <div
                style={{
                  ...
                  height: compact ? 28 : 36,
                  borderRadius: 8,
                  background: msg.isMamba ? 'var(--bg)' : (msg.avatarColor || 'var(--bg-cream)'),
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  flexShrink: 0,
              ...
              >
                {msg.isMamba ? (
                  ...
                  <span style={{ fontSize: compact ? 10 : 12, fontWeight: 700, color: 'var(--text-muted)' }}>{msg.initials}</span>
                )}
                {msg.isMamba && (
                  <span
                    style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: compact ? 12 : 13, color: 'var(--text)', lineHeight: 1.5 }}>
